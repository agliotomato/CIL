...
## 1. 연구 배경 및 동기
* **해결하고자 하는 문제:**
  * MLLM 지속 학습 시 발생하는 **치명적 망각(Catastrophic Forgetting)** 방지 — 가소성(새 학습)과 안정성(이전 지식 보존)의 균형 달성.
* **기존 연구의 한계:**
  * **구조 기반:** 추론 연산 오버헤드 및 미학습 작업 일반화 저하.
  * **리허설 기반:** 과거 데이터 보관으로 프라이버시 문제·메모리 오버헤드 발생.
  * **규제 기반:** 매개변수 직교성만으로는 실질적 그래디언트 간섭 방지 불가.
  * **기존 그래디언트 직교화:** 효과적이나 과거 데이터 의존성 존재.
...
* **상세 아키텍처 및 알고리즘:**
  * 테일러 전개로 무손실 조건 = 그래디언트 직교성임을 수학적으로 도출. 이전 가중치 $\theta_j'$에 현재 데이터를 통과시켜 GPWC 산출 → 현재 업데이트 $\theta_i$와 직교 제약 부과.
...
## 3. 실험 및 결과
* **실험 설정:**
  * UCIT 벤치마크 — 6개 도메인(ImageNet-R, ArXivQA, VizWiz, IconQA, CLEVR-Math, Flickr30k) 순차 학습. 지표: Last / Avg / BWT.
* **주요 결과:**
  * 기존 SOTA(HiDe-LLaVA) 대비 **Avg +2.14%p / Last +6.82%p** 향상, 과거 데이터 보관 Rehearsal 방식(Last 68.44%)도 초월(Last 71.01%).
  * GPWC 직교 시 BWT **+0.41**(긍정 전이), 2단계 전략 적용 여부에 따라 **9.83%p** 성능 차이 발생.

---

## 4. 토의 및 결론
* **논문의 핵심 기여:**
  * 테일러 전개로 그래디언트 직교성이 진정한 무손실 조건임을 이론적으로 증명, GPWC로 과거 데이터 없이 이를 실현, 2단계 전략으로 규제-학습 경쟁 문제 해소.
* **장점 및 한계점:**
  * **장점:** 과거 데이터 불필요(프라이버시·메모리 해소), 추가 모듈 없이 단일 LoRA 유지, 태스크 순서에 강건.
  * **한계:** 태스크 수 증가 시 GPWC 연산 비용 선형 증가, $\lambda$ 하이퍼파라미터 민감, 다양한 CL 시나리오 검증 미흡.
* **개인적인 소감:**
  * 과거 가중치를 과거 데이터의 프록시로 활용한다는 발상이 단순하면서도 강력함. 2단계 전략은 다른 규제 기반 CL 방법에도 범용적으로 적용 가능해 보임.
...
* **연구 목적 및 배경 (Objective & Background):**
  * 다중 모달 거대 언어 모델(MLLM)의 지속 학습(Continual Learning) 시 발생하는 **치명적 망각(Catastrophic Forgetting)** 방지 및 가소성(새 학습)-안정성(이전 지식) 간의 최적 균형 확보.
* **기존 연구의 한계 (Limitations of Prior Works):**
  * **구조 기반 (Architecture-based):** 작업별 LoRA 등을 할당하나 미학습 작업에 대한 일반화 성능 저하 및 추론 연산 오버헤드 유발.
  * **리허설 기반 (Rehearsal-based):** 과거 작업 데이터를 버퍼에 보존하므로 실무 환경에서 개인정보 침해(Privacy) 우려와 메모리 저장 비용 발생.
  * **규제 기반 (Regularization-based):** 매개변수 공간의 직교성(Parameter Orthogonality)에 의존하지만, 매개변수 직교성만으로는 매개변수 간 그래디언트 간섭 방지 불가.
  * **기존 그래디언트 직교화:** 간섭 방지를 위해 그래디언트 직교성이 핵심임을 알지만, 기존 방식들은 과거 역사적 데이터(Historical Data) 보존에 의존하는 한계 존재.
* **본 논문의 제안 - Octopus:**
  1. **HiFGO (History-Free Gradient Orthogonalization):**
     * 과거 데이터 없이, **과거 가중치(Weights)와 현재 데이터만을 이용**해 그래디언트 공간에서의 직교성 제약(GPWC: Gradients of Previous parameters Within Current data distribution)을 효과적으로 유도 및 강제.
  2. **2단계 미세조정 전략 (Two-stage Finetuning):**
     * 규제 조건이 원래 목적 함수 최적화를 방해(Competition)하여 성능 한계를 유발하는 경쟁 문제 해결.
     * **1단계 (Stage-1 Finetuning):** 제약 조건 없이 현재 데이터로 학습하여 로컬 최적해 영역에 빠르게 도달.
     * **2단계 (Stage-2 Finetuning):** HiFGO 직교 규제 및 L2 Regularization을 활성화하여 최적 매개변수 영역 근처에서 정교하게 미세조정(Refinement).
* **실험적 성과 (Results):**
  * UCIT 벤치마크 실험 결과, 기존 SOTA 대비 평균 성능(Avg) 2.14%, 최종 성능(Last) 6.82% 향상. 과거 데이터를 사용하지 않으면서도 멀티태스크 학습 성능에 근접하는 우수성 입증.

### Related Work

#### 1. 매개변수 효율적 모델 적응 (Parameter-Efficient Model Adaptation, PEFT)
새로운 작업마다 거대한 MLLM을 전체 미세조정(Full Fine-Tuning)하는 것은 저장 공간 및 연산 비용 측면에서 비효율적임에 따른 다양한 PEFT 기법 제안.
* **어댑터 기반 (Adapters & AdaptFormer):** 사전 학습된 네트워크에 소규모 학습 모듈을 추가하지만, 모델 복잡도가 증가하고 작업 간의 완전한 격리(Isolation) 보장 불가.
* **프롬프트 기반 (Prompt & Prefix-Tuning):** Transformer 레이어에 학습 가능한 soft prompt 벡터를 삽입하지만, 입력 수준의 제어에 국한되어 복잡한 작업에서의 표현력(Expressiveness) 한계 존재.
* **로라 (LoRA, Low-Rank Adaptation):** 가중치 업데이트를 저차원 행렬(Low-rank matrices)로 분해하여 학습 효율 극대화. MLLM 미세조정의 주류 및 전체 미세조정 대비 적은 망각 현상 입증.
  * **기본 수식 (Weight Update):** 사전 학습된 매개변수 행렬 $W_0 \in \mathbb{R}^{d \times k}$에 대해, 가중치 업데이트 $\Delta W$를 두 개의 저차원 행렬 $A \in \mathbb{R}^{r \times k}$와 $B \in \mathbb{R}^{d \times r}$ ($r \ll \min(d, k)$)의 곱으로 분해하여 최적화:
    $$\Delta W = B A$$
  * **조정된 가중치 (Adapted Weight):** 
    $$W = W_0 + \Delta W = W_0 + B A$$
    학습 중 기저 모델의 $W_0$는 고정(Freeze)되고, 오직 $A$와 $B$만 업데이트.

#### 2. 전통적인 지속 학습 기법 (Conventional Continual Learning)
일반적인 딥러닝 모델의 지속 학습에서 망각 완화를 위해 제안된 고전적 방법론.
* **규제 기반 (Regularization-based):** EWC, LwF 등은 이전 작업에서 중요도가 높았던 매개변수의 업데이트에 패널티를 부여하여 지식 보존.
* **리허설/재학습 기반 (Rehearsal/Replay-based):** DER, DGR, GEM 등은 과거 데이터나 가상 데이터를 보존/재현하고 재학습 과정에 포함하여 이전 지식 유실 완화.
* **구조 기반 (Architecture-based):** Piggyback 등과 같이 태스크가 추가될 때마다 전용 모듈이나 매개변수를 동적으로 확장하는 방식.

#### 3. MLLM을 위한 지속 학습 (Continual Learning for MLLMs)
최근 다중 모달 모델(MLLM)의 특성을 반영하여 확장된 지속 학습 연구 (Octopus 와의 결정적 차별점 제시).
* **MoE 기반 (HiDe-LLaVA, MoILE 등):** 라우팅(Routing) 메커니즘을 통해 작업별 전문가(Expert) 모듈을 선택적으로 활성화. 추가적인 추론 연산 비용 및 다수의 LoRA 파일 저장 공간 오버헤드 발생.
* **그래디언트 직교화 기반 (Orthogonal Gradient Strategies):**
  * **OGD (Orthogonal Gradient Descent):** 업데이트 방향을 이전 그래디언트들의 직교 공간으로 제한하여 간섭 방지. 다만 **이전 작업의 데이터를 계속 저장하고 참조(Rehearsal)해야 하는 한계** 존재.
  * **대안 기법 (OLoRA, BiLoRA, InfLoRA 등):** 그래디언트 저장 부담을 줄이기 위해 매개변수 벡터 등으로 대체(Projection)하여 데이터 저장 문제를 해결하려 했으나, 효과가 제한적이며 가소성-안정성 트레이드오프 문제 직면.

### Preliminaries

#### 1. LoRA
LoRA는 거대한 사전 학습 모델(PTM)을 효율적으로 Fine-tuning하기 위한 대표적인 매개변수 효율적 학습 기법(PEFT).
* **개념:** 사전 학습된 가중치 행렬 $W_0 \in \mathbb{R}^{d \times k}$를 직접 최적화하는 대신, 가중치 업데이트 양 $\Delta W$를 두 개의 저차원 행렬 $A$와 $B$의 곱으로 모델링하여 학습 매개변수의 대폭 축소.
* **기본 수식 (Weight Update):**
  $$\Delta W = B A$$
...
지속 학습(CL)은 일련의 작업 시퀀스 $T_1, T_2, \dots, T_N$을 순차적으로 학습하면서, 이전에 학습한 작업의 성능을 유지하는 동시에 새로운 지식을 통합하는 것을 목표로 설정.
* **정의:** 모델 매개변수를 $\theta$, 작업 $T_i$의 데이터셋을 $\mathcal{D}_i = \{(x_k, y_k)\}$라 할 때, 모델 $f_\theta$는 다음 목적 함수 극대화 학습:
...
* **핵심 과제:** 
  새로운 작업을 배우는 능력인 **가소성(Plasticity)**과 기존 지식을 지키는 능력인 **안정성(Stability)**의 균형을 이루어야 하나, 순차 최적화 시 기존 가중치가 훼손되어 발생하는 **치명적 망각(Catastrophic Forgetting, CF)** 억제가 핵심 과제.

### Analysis on Parameter Interference (매개변수 간섭 분석)

#### 1. 기본 표기법 (Notations) 정의
수식 이해를 돕기 위한 논문의 핵심 기호 정의:
* $W_0$: MLLM의 사전 학습된 원래 가중치 (Frozen)
* $\theta_i$: 작업(Task) $i$를 미세조정(Fine-tuning)하여 학습된 LoRA 매개변수
...
#### 2. 무손실 조건 (Lossless Condition)의 정의
...
* **수식적 표현 (식 4):** 작업 2 가중치가 가산된 후에도 작업 1 데이터셋 $\mathcal{D}_1$에서의 손실 값이 이전과 완전히 동일함으로 정의.
  $$\mathcal{L}_{\mathcal{D}_1}(\theta_1') = \mathcal{L}_{\mathcal{D}_1}(\theta_1' + \theta_2)$$

#### 3. 테일러 전개(Taylor Expansion)를 통한 수식 단순화
* **테일러 1차 근사 전개 (식 5):** 식 (4)의 우변을 현재 가중치 상태 $\theta_1'$을 기준으로 전개:
  $$\mathcal{L}_{\mathcal{D}_1}(\theta_1' + \theta_2) = \mathcal{L}_{\mathcal{D}_1}(\theta_1') + \langle \frac{\partial \mathcal{L}_{\mathcal{D}_1}(\theta_1')}{\partial \theta_1'}, \theta_2 \rangle + \mathcal{O}(||\theta_2||^2)$$
...
* **수렴된 무손실 조건 (식 6):** 무손실 조건을 만족하기 위해 최종적으로 두 벡터의 내적 0 수렴 필요.
  $$\langle \frac{\partial \mathcal{L}_{\mathcal{D}_1}(\theta_1')}{\partial \theta_1'}, \theta_2 \rangle = 0$$
* **💡 물리적 의미:** **작업 1 데이터로 계산한 순간 그래디언트 방향**과 **새로 학습할 작업 2의 가중치 업데이트 방향($\theta_2$)**이 서로 **직교(Orthogonal)**해야만 이전 지식의 완벽한 방지 가능.

#### 4. 기존 '매개변수 직교성 (Parameter Orthogonality)'의 한계
EWC, O-LoRA 등 기존 규제 방식은 가중치 매개변수 자체를 직교하도록 제약($\langle \theta_1, \theta_2 \rangle = 0$) 부과. 단, 저자들은 망각 완화 실패를 지적:
* **누적 궤적 vs 순간 그래디언트:** 학습된 가중치 $\theta_1$은 $W_0$에서 로컬 최적점(Local optimum)까지 이동한 **누적 궤적(Trajectory)**인 반면, 무손실 조건(식 6)에서 요구하는 것은 최적점에서의 **순간적인 그래디언트 방향에 해당**.
...
참 그래디언트(True Gradient)를 구해 직교화를 수행하는 기존 OGD 등의 방식은 다음 두 가지 한계 존재:
...
* **해결책 (GPWC 제안):** **현재 데이터 분포 내에서 이전 매개변수들의 그래디언트(Gradients of Previous parameters Within Current data distribution, GPWC)**를 통해 작업 간의 간섭을 정량화.
* **핵심 분석 (왜 현재 데이터로 과거 가중치를 테스트하는가?):**
  * 이전 작업의 가중치 $\theta_j'$들은 이미 해당 도메인에서 최적화(수렴)를 마친 상태임.
  * 이 수렴된 가중치에 **현재 새로운 데이터 $\mathcal{D}_i$**를 통과시켜 그래디언트를 산출.
  * 이때 검출되는 그래디언트 방향(GPWC)은 두 작업이 공유하는 지식 영역을 보여주는 동시에, "현재 데이터를 학습할 때 가중치를 이 방향으로 움직이면 이전 지식이 망가진다"는 최적화 충돌 방향을 지시.

#### 2. 그래디언트 직교 규제 (Equation 7)
* **목표:** 현재 매개변수 $\theta_i$와 GPWC 간의 직교성을 강제하는 그래디언트 직교성 제약 조건 도입을 통한 간섭 완화.
* **손실 함수 수식 (식 7):**
  $$\mathcal{L}_{orth}(\theta_i) = \sum_{j=1}^{i-1} \left( \frac{\partial \mathcal{L}_{\mathcal{D}_i}(\theta_j')}{\partial \theta_j'} \right)^T \theta_i$$
* **수식 세부 분석:**
  * $\theta_j' = W_0 + \sum_{m=1}^{j} \theta_m$: 작업 $j$까지 누적된 과거 가중치 합.
  * $\frac{\partial \mathcal{L}_{\mathcal{D}_i}(\theta_j')}{\partial \theta_j'}$: **현재 데이터 $\mathcal{D}_i$**를 사용해 과거 가중치 $\theta_j'$ 기준으로 미분하여 구한 GPWC.
...
* **성능 저하의 원인 분석:**
  1. 규제 제약으로 인한 매개변수 유효 Search Spaced의 축소.
  2. 본래 작업 학습 목적 함수($\mathcal{L}_{ce}$)와 망각 방지 규제 함수($\mathcal{L}_{orth}$, $\mathcal{L}_{norm}$) 간의 간섭으로 Suboptimal Local Minima에 수렴.
...
* **손실 함수 (식 9):**
  $$\mathcal{L}_1 = \frac{1}{|\mathcal{D}_i|} \sum_{(x_k, y_k) \in \mathcal{D}_i} \mathcal{L}_{ce}(f_{\theta_{i,1}'}(x_k), y_k)$$
...
* **손실 함수 (식 10):**
  $$\mathcal{L}_2 = \frac{1}{|\mathcal{D}_i|} \sum_{(x_k, y_k) \in \mathcal{D}_i} \left( \mathcal{L}_{ce}(f_{\theta_{i,2}'}(x_k), y_k) + \lambda_1 \mathcal{L}_{orth}(\theta_{i,2}) + \lambda_2 \mathcal{L}_{norm}(\theta_{i,2}) \right)$$
...
* **벤치마크 (UCIT):** MLLM의 실질적인 지속 학습 능력을 평가하기 위해 고안된 벤치마크. 다음의 6가지 고유한 도메인과 데이터셋이 순차적으로 모델에 입력되어 도메인 변화와 치명적 망각을 유도:
...
* **주요 평가 지표 (Metrics):**
  ...
  * **BWT (Backward Transfer, 역전이):** 이전 태스크들의 성능 저하(망각 정도)를 보여주며, **양이 될수록 성능이 오히려 향상(Positive Transfer)**되었음을 의미.
...
* **Table 1 수치적 비교 (Avg & Last):**
  ...
  * **Vanilla Rehearsal 방식 초월:**
    ...
    * 이는 단순 망각 방지를 넘어 태스크 간 긍정적인 지식 전이(Positive Transfer)가 유도되었음을 의미.
