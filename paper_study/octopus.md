...
* **기존 연구의 한계:**
  * **Architecture-based:** 작업별 LoRA 등을 할당하나 미학습 작업에 대한 일반화 성능 저하 및 추론 연산 오버헤드 유발.
  * **Rehearsal-based:** 과거 작업 데이터를 버퍼에 보존하므로 실무 환경에서 개인정보 침해(Privacy) 우려와 메모리 저장 비용 발생.
  * **Regularization-based:** 매개변수 공간의 직교성(Parameter Orthogonality)에 의존하지만, 매개변수 직교성만으로는 매개변수 간 그래디언트 간섭 방지 불가.
...
* **본 논문의 제안 - Octopus:**
  ...
  2. **2단계 미세조정 전략 (Two-stage Finetuning):**
     ...
     * **2단계 (Stage-2 Finetuning):** HiFGO 직교 규제 및 L2 Regularization을 활성화하여 최적 매개변수 영역 근처에서 정교하게 미세조정
...
* **LoRA:** 가중치 업데이트를 Low-rank matrices로 분해하여 학습 효율 극대화. 
...
* **해결책 (GPWC 제안):** **현재 데이터 분포 내에서 이전 매개변수들의 그래디언트**를 통해 작업 간의 간섭을 정량화.
